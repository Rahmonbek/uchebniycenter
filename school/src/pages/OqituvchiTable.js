                ...
                {attendance.map((val, key5) => {
                  return number == val.group ? (
                    <Col lg={2} style={{ paddingTop: "15px" }}>
                      ...
                      <NeuTextInput type="date" color="#E4EBF5" onChange={(newValue) => getDate(newValue)} width="123px" height="30px" distance={1} value={val.day} fontSize={12} fontColor="#4CAF50" />
                      {studentBygroup && Array.isArray(studentBygroup)
                        ? studentBygroup.map((item, key) => {
                            return (
                              <div style={{ marginBottom: "15px", marginTop: "10px" }}>
                                {val.students.map((item3) => {
                                  return item.group == val.group ? <Checkbox checked={item.id == item3} onClick={() => get(item)} color="var(--success)" style={{ display: "block" }} /> : "";
                                })}
                              </div>
                            );
                          })
